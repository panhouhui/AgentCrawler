...
class InstaloaderContext:
    ...
    def web_profile_info(self, username: str) -> Dict[str, Any]:
        """
        Retrieve a profile's metadata from the ``web_profile_info`` endpoint.

        This endpoint can be queried anonymously as well as when logged in and returns
        the complete profile node in the legacy format, including the first page of
        timeline posts. It replaces the GraphQL search/profile queries that Instagram
        started responding to with HTTP 400.
        ...
        with copy_session(self._session, self.request_timeout) as tmpsession:
            ...
            del tmpsession.headers['Connection']
            del tmpsession.headers['Content-Length']
            # The www.instagram.com web app id is required for this endpoint; the
            # default app id (x-ig-app-id) is rejected with HTTP 400.
            tmpsession.headers['x-ig-app-id'] = '936619743392459'
            tmpsession.headers['accept'] = '*/*'
            tmpsession.headers['referer'] = 'https://www.instagram.com/{0}/'.format(username)
            return self.get_json('api/v1/users/web_profile_info/',
                                 params={'username': username},
                                 session=tmpsession)

    def web_user_info(self, user_id: Union[int, str]) -> Dict[str, Any]:
        """
        Retrieve a user's basic info (notably the current username) from a user id.

        Works anonymously as well as when logged in. Used to resolve a profile by id,
        replacing the GraphQL user query that Instagram started responding to with
        HTTP 400.

        :param user_id: User id (pk).
        :return: The server's response dictionary.
        ...
        with copy_session(self._session, self.request_timeout) as tmpsession:
            ...
            tmpsession.headers['x-ig-app-id'] = '936619743392459'
            tmpsession.headers['accept'] = '*/*'
            return self.get_json('api/v1/users/{0}/info/'.format(user_id),
                                 params={},
                                 session=tmpsession)
