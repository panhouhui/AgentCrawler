...
class BrowserManager:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # Launch browser. Prefer Playwright-managed Chromium, but fall back to
            # system Edge when the matching Chromium browser has not been installed.
            try:
                ...
                can_fallback_to_edge = (
                    "channel" not in self.launch_options
                    and "executable_path" not in self.launch_options
                    and (
                        "Executable doesn't exist" in str(launch_error)
                        or "playwright install" in str(launch_error)
                    )
                )
                if not can_fallback_to_edge:
                    raise

                logger.warning(
                    "Playwright Chromium is unavailable; falling back to system Microsoft Edge."
                )
                self._browser = await self._playwright.chromium.launch(
                    headless=self.headless,
                    slow_mo=self.slow_mo,
